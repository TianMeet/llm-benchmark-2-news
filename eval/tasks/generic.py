...
任务 YAML 结构：
  name: str
  version: str
  prompt_template: str          # eval/prompts/ 下的模板文件名（不含 .yaml）
...
# 辅助函数已迁移至 eval/task_metrics/calculators.py，此处保留别名以向后兼容。
_normalize_value = normalize_value
_safe_float = safe_float
_extract_text_fragments = extract_text_fragments
_as_set_of_str = as_set_of_str
...
class GenericTask(EvalTask):
    ...
    def metrics(
        ...
        """按 metrics 配置计算指标，分发到 eval.task_metrics.compute_metric。"""
