...
def generate_report(run_dir: str | Path) -> Path:
    ...
    for r in errors:
        error_type_stats[str(r.get("error_type", "") or "unknown_error")] += 1
    ...
    if error_type_stats:
        lines.extend(
            [
                ...
                "",
                _markdown_table(
                    [{"error_type": k, "count": v} for k, v in sorted(error_type_stats.items())],
                    ["error_type", "count"],
                ),
                "",
            ]
    ...
    for idx, err in enumerate(errors[:5], start=1):
        lines.extend(
            [
                ...
                f"- error_type: `{err.get('error_type', '')}` / stage: `{err.get('error_stage', '')}`",
